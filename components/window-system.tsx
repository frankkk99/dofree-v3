  ...
  try {
    ...
    if (host === 'youtube.com' || host === 'm.youtube.com') {
      if (parsed.pathname.startsWith('/embed/')) {
        return appendQuery(url, { autoplay: '1', rel: '0', playsinline: '1' });
      }
...
export function SearchWindow({
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const haystack = [item.title, item.titleEn, item.year, item.mediaType, ...(item.genres || [])]
        .join(' ')
        .toLowerCase();
    ...
    <div className="fixed inset-0 z-[95] overflow-y-auto bg-black/88 px-3 py-4 text-white backdrop-blur-2xl" role="dialog" aria-modal="true">
      <div className="mx-auto max-w-4xl rounded-[28px] bg-[#050505]/92 p-4 shadow-[0_38px_120px_rgba(0,0,0,0.9)] backdrop-blur-2xl md:p-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-white/[0.08] text-xl font-black text-white/80 shadow-[0_12px_36px_rgba(0,0,0,0.55)] backdrop-blur-xl hover:bg-white/[0.14]">
            ×
          </button>
        </div>
        ...
        {visibleItems.length < filteredItems.length ? (
          <div ref={loadMoreRef} className="py-5 text-center text-[11px] font-black text-white/35">
            กำลังโหลดเพิ่ม...
          </div>
        ) : null}
      </div>
    ...
    <div onClick={onClose} className="fixed inset-0 z-[90] overflow-y-auto bg-black/70 px-2 py-4 text-white backdrop-blur-2xl md:bg-black/78 md:px-4 md:py-7" role="dialog" aria-modal="true">
      <div onClick={(event) => event.stopPropagation()} className="mx-auto w-full max-w-[860px] overflow-hidden rounded-[28px] bg-[#050505]/88 shadow-[0_42px_150px_rgba(0,0,0,0.94)] backdrop-blur-2xl md:rounded-[34px]">
        ...
        <div className="relative bg-black/42 shadow-[inset_0_-80px_110px_rgba(0,0,0,0.55)] backdrop-blur-xl">
          ...
          <button onClick={onClose} className="absolute right-3 top-3 z-20 grid h-9 w-9 place-items-center rounded-full bg-black/62 text-xl font-black text-white/80 shadow-[0_14px_42px_rgba(0,0,0,0.72)] backdrop-blur-xl hover:bg-white/[0.12] md:right-4 md:top-4 md:h-11 md:w-11 md:text-2xl" aria-label="ปิดรายละเอียด">
            ×
          </button>
          ...
          <div className="relative z-10 grid grid-cols-[88px_1fr] gap-3 p-4 pt-5 md:grid-cols-[118px_1fr] md:gap-4 md:p-5 md:pb-4">
            ...
            <div className="min-w-0 pr-9 md:pr-11">
              <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#e50914] md:text-[10px] md:tracking-[0.24em]">
                {displayItem.mediaType === 'tv' ? 'Series' : 'Movie'}
              </p>
              <h2 className="modal-title mt-1.5 line-clamp-2 text-[21px] font-black leading-[0.98] tracking-[-0.06em] text-white md:text-[31px]">
                {displayItem.title}
              </h2>
              ...
              <p className={`${expanded ? '' : 'line-clamp-3'} mt-2 text-[11px] font-medium leading-4 text-white/58 md:text-[13px] md:leading-5`}>
                {displayItem.overview}
              </p>

              <button onClick={() => setExpanded((value) => !value)} className="mt-1 text-[10px] font-black text-red-200/80 hover:text-red-100 md:text-xs">
                {expanded ? 'ย่อ' : 'ดูเพิ่มเติม'}
              </button>

              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  onClick={() => displayItem.watchUrl && setActiveTab('watch')}
                  disabled={!displayItem.watchUrl}
                  className={`inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-[11px] font-black text-white shadow-[0_14px_36px_rgba(0,0,0,0.42)] backdrop-blur-xl md:h-9 md:px-4 md:text-xs ${
                    displayItem.watchUrl ? 'bg-[#e50914]' : 'bg-white/[0.09] text-white/40'
                  }`}
                >
                  ▶ รับชม
                </button>
                <button onClick={() => setActiveTab('recommend')} className="inline-flex h-8 items-center rounded-lg bg-white/[0.1] px-3 text-[11px] font-black text-white/82 shadow-[0_14px_36px_rgba(0,0,0,0.32)] backdrop-blur-xl hover:bg-white/[0.16] md:h-9 md:px-4 md:text-xs">
                  + รายการโปรด
                </button>
              </div>
            </div>
          ...
          <div className="relative z-10 px-4 pb-4 md:px-5 md:pb-5">
            ...
            <InlinePlayer
              url={displayItem.trailerUrl}
              title={`ตัวอย่าง ${displayItem.title}`}
              fallbackImage={displayItem.backdropUrl || displayItem.posterUrl}
              emptyLabel={detailLoading ? 'กำลังโหลดตัวอย่างจาก TMDB...' : 'ยังไม่มีตัวอย่างที่ฝังใน Modal ได้'}
            />
          </div>
        </div>
          ...
          <div className="movie-rail flex gap-1 overflow-x-auto py-2.5 md:gap-1.5 md:py-3">
            {modalTabs.map((tab) => (
              <button
                ...
                className={`min-w-max rounded-md px-3 py-1.5 text-[10px] font-black transition shadow-[0_12px_32px_rgba(0,0,0,0.28)] backdrop-blur-xl md:px-4 md:py-2 md:text-xs ${
                  activeTab === tab.id ? 'bg-[#e50914] text-white shadow-glow' : 'bg-white/[0.07] text-white/55 hover:bg-white/[0.12] hover:text-white'
                }`}
              >
                {tab.label}
              </button>
          ...
          <Surface className="min-h-[320px] p-4 md:p-5">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  {selectedPerson ? (
                    <button
                      ...
                      className="rounded-full bg-white/[0.08] px-3 py-1 text-[10px] font-black text-white/68 shadow-[0_10px_28px_rgba(0,0,0,0.32)] backdrop-blur-xl hover:bg-white/[0.14] md:text-xs"
                    >
                      กลับไปนักแสดง
                    </button>
                  ) : detailLoading ? (
                ...
                {!selectedPerson ? (
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2 md:grid-cols-4 md:gap-3" aria-hidden="true">
                    {Array.from({ length: 9 }).map((_, index) => (
                      <div key={index} className="aspect-[2/3] animate-pulse rounded-[12px] bg-white/[0.055] md:rounded-[16px]" />
                    ))}
                  </div>
                ) : personMovies.length ? (
                  <div className="mt-4 grid grid-cols-3 gap-2 md:grid-cols-4 md:gap-3">
                    {personMovies.map((movie, index) => (
                      <MovieCard
                        key={`actor-${selectedPerson.id}-${movie.mediaType}-${movie.id}-${index}`}
                        item={movie}
                        grid
                        compact
                        onSelect={(nextItem) => {
                        ...
                        priorityBadge={index < 3 ? 'ผลงาน' : undefined}
                      />
                    ))}
                  </div>
                ) : (
                  <div className="mt-4 rounded-2xl bg-black/28 p-4 text-center text-xs font-bold text-white/45 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] backdrop-blur-xl md:text-sm">
                    ยังไม่พบผลงานของนักแสดงคนนี้จาก TMDB
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'recommend' && (
              <div>
                ...
                {visibleRecommendations.length < detailRecommendations.length ? (
                  <div ref={recLoadRef} className="py-4 text-center text-[10px] font-black text-white/35">
                    กำลังโหลดเพิ่ม...
                  </div>
                ) : null}
              </div>
            ...
            {activeTab === 'spoiler' && (
              <div>
                ...
                <div className="mt-3 rounded-2xl bg-yellow-300/[0.08] p-3 text-xs leading-5 text-yellow-50/75 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl md:text-sm md:leading-6">
                  {displayItem.overview}
                </div>
              </div>
            )}
            ...
            {activeTab === 'watch' && (
              <div>
                ...
                <div className="mt-3">
                  <InlinePlayer
                    url={displayItem.watchUrl}
                    title={`รับชม ${displayItem.title}`}
                    fallbackImage={displayItem.backdropUrl || displayItem.posterUrl}
                    emptyLabel="ยังไม่มีลิงก์รับชม"
                  />
                </div>

                <div className="mt-3 flex justify-end">
                  <button onClick={reportIssue} className="h-9 rounded-xl bg-black/35 px-4 text-xs font-black text-white/70 shadow-[0_12px_34px_rgba(0,0,0,0.32)] backdrop-blur-xl md:h-10">
                    แจ้งลิงก์เสีย
                  </button>
                </div>

                {reported ? (
                  <p className="mt-3 rounded-2xl bg-green-400/[0.09] p-3 text-xs font-bold text-green-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl md:text-sm">
                    รับรายงานแล้ว ทีมแอดมินจะตรวจสอบลิงก์นี้
                  </p>
                ) : null}
              </div>
            )}
