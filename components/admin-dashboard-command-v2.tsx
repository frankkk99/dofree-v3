...
const panelClass = 'rounded-2xl border border-white/8 bg-black/35 p-4';

function Stat({ label, value, hint, accent = '' }: { label: string; value: unknown; hint: string; accent?: string }) {
  return <article className="rounded-2xl border border-white/8 bg-black/32 p-3"><p className="text-[10px] font-black text-white/42">{label}</p><p className={`mt-2 text-2xl font-black tracking-[-0.04em] ${accent}`}>{typeof value === 'string' ? value : fmt(value)}</p><p className="mt-1 text-[11px] font-semibold text-white/36">{hint}</p></article>;
}
function Empty({ text }: { text: string }) { return <div className="rounded-2xl bg-white/[0.04] p-4 text-sm font-bold text-white/42">{text}</div>; }
function Bar({ label, value, total, color = 'bg-[#e50914]' }: { label: string; value: number; total: number; color?: string }) {
  return <div><div className="mb-1 flex justify-between text-xs font-black text-white/58"><span>{label}</span><span>{pct(value, total)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-white/[0.08]"><div className={`h-full rounded-full ${color}`} style={{ width: width(value, total) }} /></div></div>;
}
function List({ rows, empty, total }: { rows?: Row[]; empty: string; total?: number }) {
  return <div className="grid gap-2">{rows?.length ? rows.map((row, index) => <div key={`${row.label || row.code || index}`} className="rounded-2xl bg-white/[0.055] px-4 py-3"><div className="flex items-center justify-between gap-3"><p className="min-w-0 truncate text-sm font-black text-white/78">{row.label || row.code || '-'}</p><p className="text-lg font-black text-[#f4c46b]">{fmt(row.value ?? row.views)}</p></div>{total ? <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/[0.08]"><div className="h-full rounded-full bg-[#f4c46b]" style={{ width: width(Number(row.value || 0), total) }} /></div> : null}</div>) : <Empty text={empty} />}</div>;
}
...
export function AdminDashboardCommandV2() {
  ...
  return <section className="mx-auto w-full max-w-7xl px-4 py-4 text-white md:px-8 md:py-6"><div className="admin-floating-glass rounded-2xl border border-white/8 p-3 md:rounded-[28px] md:p-5">
    ...
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-2 overflow-x-auto">{ranges.map((item) => <button key={item.key} onClick={() => setRange(item.key)} className={`rounded-2xl px-4 py-2 text-xs font-black ${range === item.key ? 'bg-white text-black' : 'bg-white/[0.08] text-white/55'}`}>{item.label}</button>)}</div><span className={`rounded-full px-3 py-2 text-[10px] font-black ${trackingReady ? 'bg-emerald-400/12 text-emerald-100' : 'bg-orange-400/12 text-orange-100'}`}>{trackingReady ? 'TRACKING ON' : 'SETUP NEEDED'}</span></div>
