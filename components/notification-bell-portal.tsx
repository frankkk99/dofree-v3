...
export function NotificationBellPortal() {
  ...
  useEffect(() => {
    ...
    function findHost() {
      const nextHost = document.querySelector('[data-dofree-notification-host="true"]') as HTMLElement | null;
  ...
  const button = createPortal(
    <button
      ...
      className="relative grid h-10 w-10 place-items-center rounded-none bg-transparent text-white transition hover:opacity-75 md:h-12 md:w-12"
    >
  ...
  const modal = open && bodyHost ? createPortal(
