    ...
    <section className="px-4 sm:px-5">
      ...
      <div className="relative aspect-video w-full max-w-full overflow-hidden rounded-[28px] bg-black shadow-[0_24px_90px_rgba(0,0,0,0.72)] sm:rounded-[32px]">
        {embedUrl ? (
          ...
          <>
            {fallbackImage ? <img src={fallbackImage} alt="" loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover opacity-25" /> : null}
            <div className="absolute inset-0 bg-black/65" />
            <div className="relative z-10 grid h-full place-items-center px-6 text-center">
              <div>
                <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-white/[0.1] text-xl font-black text-white/58 backdrop-blur-xl">▶</div>
      ...
      <article data-detail-layout="modal-visual-page-v7-single-watch-bottom" className="mx-auto w-full max-w-[920px] overflow-hidden rounded-[28px] bg-[#050505]/88 shadow-[0_42px_150px_rgba(0,0,0,0.94)] backdrop-blur-2xl md:rounded-[34px]">
        <section className="relative overflow-hidden bg-black/42 pb-4 shadow-[inset_0_-80px_110px_rgba(0,0,0,0.55)] backdrop-blur-xl md:pb-5">
          ...
          <div className="relative z-10 grid grid-cols-[88px_1fr] gap-3 p-4 pt-5 md:grid-cols-[126px_1fr] md:gap-5 md:p-5 md:pb-4">
            ...
            <div className="min-w-0 pr-9 md:pr-11">
              ...
              <h1 className="modal-title mt-1.5 line-clamp-2 text-[21px] font-black leading-[0.98] tracking-[-0.06em] text-white md:text-[34px]">{item.title}</h1>
              ...
              <p className="mt-2 line-clamp-3 max-w-2xl text-[11px] font-medium leading-4 text-white/58 md:text-[13px] md:leading-5">{overview}</p>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <DetailReportButton tmdbId={item.id} mediaType={item.mediaType} title={item.title} className="inline-flex h-8 items-center rounded-lg bg-white/[0.08] px-3 text-[11px] font-black text-white/70 backdrop-blur-xl transition hover:bg-white/[0.14] md:h-9 md:px-4 md:text-xs">แจ้งลิงก์เสีย</DetailReportButton>
              </div>
            </div>
          </div>
        ...
        <section className="flex flex-col gap-6 p-3 md:p-4">
          ...
          <SectionSurface id="cast">
            ...
            {cast.length ? <div className="mt-4 grid grid-cols-3 gap-2 md:grid-cols-4 md:gap-3">{cast.slice(0, 12).map((person, index) => <div key={`${person.id || person.name}-${index}`} className="relative aspect-[2/3] overflow-hidden rounded-[12px] bg-white/[0.055] shadow-[0_16px_54px_rgba(0,0,0,0.55)] backdrop-blur-xl md:rounded-[16px]">{person.profileUrl ? <img src={person.profileUrl} alt={person.name} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_50%_24%,#8a111b,#111_62%)] text-4xl font-black text-white/78 md:text-5xl">{personInitial(person.name)}</div>}<div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.04)_0%,rgba(0,0,0,0.06)_44%,rgba(0,0,0,0.92)_100%)]" /><div className="absolute inset-x-0 bottom-0 p-2 md:p-3"><h3 className="line-clamp-2 text-[10px] font-black leading-tight text-white drop-shadow md:text-sm">{person.name}</h3><p className="mt-1 line-clamp-2 text-[8px] font-bold leading-3 text-white/52 md:text-[10px] md:leading-4">{person.character || 'นักแสดง'}</p></div></div>)}</div> : <div className="mt-4 rounded-2xl bg-black/28 p-4 text-center text-xs font-bold text-white/45 backdrop-blur-xl md:text-sm">ยังไม่มีข้อมูลนักแสดงสำหรับเรื่องนี้</div>}
          </SectionSurface>
