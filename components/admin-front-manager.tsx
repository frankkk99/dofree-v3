...
import {
  ...
  emptyFrontOptions,
  type AdminFrontItem,
  type AdminFrontOptions,
  type AdminFrontPayload,
...
function buildCatalogUrl(filters: Filters, offset: number) {
...
export function AdminFrontManager() {
  ...
  const loadCatalog = useCallback(async (nextOffset: number, mode: 'replace' | 'append') => {
    ...
    try {
      const response = await fetch(buildCatalogUrl(debouncedFilters, nextOffset), {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      if (entries[0]?.isIntersecting && hasMore && !loading && !loadingMore) {
        void loadCatalog(offset, 'append');
      }
  ...
  return (
    <section className="mx-auto grid w-full max-w-7xl gap-5 px-4 py-6 text-white md:px-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
        {items.map((item) => (
          <AdminFrontManagerCard key={`${item.media_type}-${item.tmdb_id}-${item.section_slug || item.source_bucket || 'item'}`} item={item} onSelect={setSelected} />
        ))}
      </div>
      ...
      {selected ? <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm font-bold text-white/70">เลือก: {adminFrontItemName(selected)} — panel แก้ไขจะเพิ่มในชุดถัดไป</div> : null}
