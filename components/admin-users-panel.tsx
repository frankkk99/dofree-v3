...
export function AdminUsersPanel() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-8 text-white md:px-8">
      <div className="rounded-[36px] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(229,9,20,0.18),transparent_34%),rgba(255,255,255,0.045)] p-5 shadow-[0_30px_120px_rgba(0,0,0,0.62)] md:p-8">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <a href="/admin" className="text-xs font-black text-[#e50914] hover:text-red-300">← กลับแดชบอร์ด</a>
            <p className="mt-5 text-[11px] font-black uppercase tracking-[0.30em] text-[#e50914]">Users & Roles</p>
            <h1 className="mt-3 text-4xl font-black tracking-[-0.08em] md:text-7xl">จัดการผู้ใช้</h1>
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-7 text-white/52 md:text-base">
              ดูรายชื่อผู้ใช้ เปลี่ยน role viewer/admin และดูสัญญาณการใช้งานจาก Favorites, History และ Membership
            </p>
          </div>
          <button onClick={loadUsers} className="rounded-2xl bg-[#e50914] px-5 py-3 text-sm font-black text-white shadow-glow">Refresh</button>
        </div>

        {stats ? (
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-[26px] border border-white/10 bg-black/35 p-5"><p className="text-xs font-bold text-white/36">ผู้ใช้ทั้งหมด</p><p className="mt-2 text-4xl font-black">{stats.total}</p></div>
            <div className="rounded-[26px] border border-white/10 bg-black/35 p-5"><p className="text-xs font-bold text-white/36">Admin</p><p className="mt-2 text-4xl font-black text-[#e50914]">{stats.admins}</p></div>
            <div className="rounded-[26px] border border-white/10 bg-black/35 p-5"><p className="text-xs font-bold text-white/36">Viewer</p><p className="mt-2 text-4xl font-black">{stats.viewers}</p></div>
            <div className="rounded-[26px] border border-white/10 bg-black/35 p-5"><p className="text-xs font-bold text-white/36">Premium Active</p><p className="mt-2 text-4xl font-black text-[#f4c46b]">{stats.activePremium}</p></div>
          </div>
        ) : null}
        ...
        {loading ? (
          ...
          <div className="mt-6 overflow-hidden rounded-[28px] border border-white/10 bg-black/35">
            ...
            <div className="divide-y divide-white/8">
              {filtered.map((user) => (
                <article key={user.id} className="grid gap-4 px-4 py-4 lg:grid-cols-[1.15fr_0.75fr_0.8fr_0.8fr_1fr] lg:items-center">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="rounded-xl bg-white/[0.08] px-4 py-2 text-xs font-black text-white/70 disabled:cursor-not-allowed disabled:opacity-35"
                    >
                      Viewer
                    </button>
                    <button
                      ...
                      className="rounded-xl bg-[#e50914] px-4 py-2 text-xs font-black text-white shadow-glow disabled:cursor-not-allowed disabled:opacity-35"
                    >
                      Admin
                    </button>
                  </div>
