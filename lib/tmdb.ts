...
function youtubeTrailer(videos: TmdbVideo[] = []) {
  ...
  const video = youtubeVideos.find((item) => item.type === 'Trailer' && item.official) || youtubeVideos.find((item) => item.type === 'Trailer') || youtubeVideos.find((item) => item.type === 'Teaser') || youtubeVideos.find((item) => item.type === 'Clip') || youtubeVideos[0];
...
export async function getMediaImagePayload(mediaType: MediaType, id: string | number): Promise<MediaImagePayload> {
  ...
  if (!numericId) return { posterUrl: fallbackImages[0], backdropUrl: fallbackImages[1], source: 'fallback' };
  const detail = await tmdb<TmdbItem>(`/${mediaType}/${numericId}?language=th-TH`);
  if (!detail?.id) return { posterUrl: fallbackImages[numericId % fallbackImages.length], backdropUrl: fallbackImages[(numericId + 1) % fallbackImages.length], source: 'fallback' };
  return {
